...
struct SessionInspectorView: View {
    ...
    private func updateHighlightedFiles() {
        ...
        if let end = tc.selEnd {
            ...
            let tHi = max(tc.selTrack ?? 0, tc.selTrackEnd ?? tc.selTrack ?? 0)
            var files = Set<String>()
            for ti in tLo...tHi where ti < tracks.count {
                for clip in tracks[ti].clips where !clip.isGroup {
                    let cEnd = clip.startSample + clip.lengthSamples
                    if clip.startSample < hi && cEnd > lo && !clip.sourceFile.isEmpty {
...
private final class TimelineController: ObservableObject, @unchecked Sendable {
    ...
    private func toggle(_ set: inout Set<Int>, _ i: Int) {
        ...
        audibleDidChange?(currentAudibleSet())   // let an in-progress transport update live
    }
    func toggleMute(_ i: Int) { toggle(&mutedTracks, i) }
    func toggleSolo(_ i: Int) { toggle(&soloTracks, i) }
    ...
    func seedMute(from tracks: [PTXTrack]) {
        mutedTracks = Set(tracks.indices.filter { tracks[$0].isMuted || tracks[$0].isInactive })
    }
