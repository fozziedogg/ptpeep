    ...
    static func buildTrackPlaylists(blocks: [PTXBlock], data: Data, bigEndian: Bool,
        ...
        for c in cmpdParents {
            ...
            for (_, counters) in slotCounterLists where counters.count > 1 {
                ...
                // Search 0x2628 at 95-byte strides for the sorted ordinal sequence
                var ordinalArrayStart: Int? = nil
                outerLoop: for off in Swift.stride(from: 0, through: nb.dataSize - recordStride * N, by: recordStride) {
