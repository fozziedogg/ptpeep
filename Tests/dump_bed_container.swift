#!/usr/bin/env swift
// Dump all block types inside 0x261b containers for BED tracks
import Foundation
...
let bedStripNames: Set<String> = ["DX 7.1 Floor", "SRC MX 7.1 Floor"]
...
for container in all261b {
    ...
    guard let stripName = String(bytes: data[(nameOff+4)..<nameEnd], encoding: .utf8),
          bedStripNames.contains(stripName) else { continue }

    // Find output path to confirm this is a BED
    ...
    var outPath = ""
    for pb in routingBlocks {
        guard pb.size >= 2, !(data[pb.off] == 0xff && data[pb.off+1] == 0xff) else { continue }
        let lpOff = pb.off + 36
        guard lpOff + 4 <= pb.off + pb.size else { continue }
        let sl = Int(u32le(lpOff)); guard sl > 0, sl <= 256 else { continue }
        guard lpOff + 4 + sl <= pb.off + pb.size else { continue }
        if let s = String(bytes: data[(lpOff+4)..<(lpOff+4+sl)], encoding: .utf8), !s.isEmpty {
            outPath = s; break
        }
    }

    print("Container: \"\(stripName)\" → output: \"\(outPath)\"")
    print("  Container: off=\(cStart) size=\(container.size)")
    print()

    // Dump ALL blocks inside this container
    let inner = blocks.filter { $0.off >= cStart && $0.off + $0.size <= cEnd }
    for b in inner {
        // First 12 bytes of data
        let preview = min(12, b.size)
        let hex = (0..<preview).map { String(format: "%02x", data[b.off + $0]) }.joined(separator: " ")
        print(String(format: "    ct=0x%04x  off=%d  sz=%d  [%@]", b.ct, b.off, b.size, hex))
    }
    print()
}
