...
// Check post[18] for every track in the 0x251a list.
// Hypothesis: post[18] != 0 iff the track is top-level (depth 0).
...
struct DumpPost18 {
    ...
    static func run() {
        guard CommandLine.arguments.count > 1 else {
            fputs("Usage: dump_post18 <file.ptx>\n", stderr); exit(1)
        }
        ...
        for (i, sub) in subBlocks.enumerated() {
            ...
            let p18 = nameEnd + 18 < sub.off + sub.size ? data[nameEnd + 18] : 0xff

            let tcStr  = String(format: "0x%04x", typeCode)
            let fldStr = folderFlag != 0 ? "FLD" : "   "
            let p18Str = String(format: "%02x", p18)
            let p0Str  = String(format: "%02x", p0)
            let iStr   = String(i).padding(toLength: 5, withPad: " ", startingAt: 0)
            print("\(iStr)  \(tcStr)  \(fldStr)  \(p18Str)    \(p0Str)    \(name)")
        }
    }
