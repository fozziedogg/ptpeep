...
// Dump ALL 0x251a entries (first occurrence) with tc, p62/color, and name.
...
struct DumpAllP62 {
    ...
    static func run() {
        guard CommandLine.arguments.count > 1 else {
            fputs("Usage: dump_all_p62 <file.ptx>\n", stderr); exit(1)
        }
        ...
        for (i, sub) in subBlocks.enumerated() {
            ...
            let nameLen = Int(nl)
            let nameStart = p + 6; let nameEnd = nameStart + nameLen
            ...
            let p0 = nameEnd < sub.off + sub.size ? data[nameEnd] : 0xff
            let colorOff = p + 68 + nameLen
            let p62: Int
            if colorOff + 1 < sub.off + sub.size {
                p62 = Int(UInt16(data[colorOff]) | UInt16(data[colorOff + 1]) << 8)
            } else {
                p62 = 0xffff
            }

            let tcStr   = String(format: "0x%04x", typeCode)
            let p62Str  = String(format: "0x%04x", p62)
            let fldStr  = folderFlag ? "FLD " : "    "
            let p0Str   = String(format: "%02x", p0)
            let iStr    = String(i).padding(toLength: 5, withPad: " ", startingAt: 0)
            print("\(iStr) \(tcStr)  \(p62Str)  \(fldStr) \(p0Str)    \(name)")
        }
    }
